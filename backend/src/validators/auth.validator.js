...
export const validateRegister = (req, res, next) => {
  ...
  if (!name || typeof name !== 'string' || name.trim().length < 2 || name.trim().length > 100) {
    return next(new AppError('Please provide a valid name (between 2 and 100 characters)', HTTP_STATUS.BAD_REQUEST));
  }
...
export const validateLogin = (req, res, next) => {
  ...
  if (!password || typeof password !== 'string') {
    return next(new AppError('Please provide a valid password', HTTP_STATUS.BAD_REQUEST));
  }
...
export const validateUpdateProfile = (req, res, next) => {
  ...
  if (name !== undefined && (typeof name !== 'string' || name.trim().length < 2 || name.trim().length > 100)) {
    return next(new AppError('Name must be between 2 and 100 characters long', HTTP_STATUS.BAD_REQUEST));
  }

  if (avatar !== undefined && (typeof avatar !== 'string' || !avatar.startsWith('http'))) {
    return next(new AppError('Avatar must be a valid URL string', HTTP_STATUS.BAD_REQUEST));
  }
