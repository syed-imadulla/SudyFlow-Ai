...
export const validateCreateGoal = (req, res, next) => {
  ...
  if (!title || typeof title !== 'string' || title.trim().length === 0) {
    return next(new AppError('Goal title is required and must be a string', HTTP_STATUS.BAD_REQUEST));
  }

  if (urgency && !['URGENT', 'UPCOMING', 'ACTIVE', 'COMPLETED'].includes(urgency)) {
    return next(new AppError('Invalid urgency level provided', HTTP_STATUS.BAD_REQUEST));
  }

  if (status && !Object.values(GOAL_STATUS).includes(status.toUpperCase())) {
    return next(new AppError('Invalid goal status provided', HTTP_STATUS.BAD_REQUEST));
  }
...
export const validateUpdateGoal = (req, res, next) => {
  ...
  if (!id || !mongoose.Types.ObjectId.isValid(id)) {
    return next(new AppError('Invalid Goal ID format', HTTP_STATUS.BAD_REQUEST));
  }

  const { title, urgency, status } = req.body || {};
  if (title !== undefined && (typeof title !== 'string' || title.trim().length === 0)) {
    return next(new AppError('Goal title cannot be empty', HTTP_STATUS.BAD_REQUEST));
  }
...
export const validateToggleSubtask = (req, res, next) => {
  ...
  if (!subtaskId || !mongoose.Types.ObjectId.isValid(subtaskId)) {
    return next(new AppError('Invalid Subtask ID format', HTTP_STATUS.BAD_REQUEST));
  }
  next();
};
